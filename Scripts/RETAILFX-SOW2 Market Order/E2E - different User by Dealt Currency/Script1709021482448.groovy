...
for (def row = 1; row <= findTestData('Dealt_MYR_SGD').getRowNumbers(); row++) {
    Mobile.delay(10)
    ...
    if (checkLinkedDevice == true) {
        ...
        Mobile.setText(findTestObject('Object Repository/Dashboard_Login/Onboarding_Login/Password'), findTestData('Dealt_MYR_SGD').getValue(
    ...
    CustomKeywords.'utilities.Deal_Agregator.MarketOrder_FromDropdown'(findTestData('Dealt_MYR_SGD').getValue('Base_Currency', 
            row))
    ...
    'Select Currency'
    CustomKeywords.'utilities.Deal_Agregator.sow2market_orderdropdown'(findTestData('Dealt_MYR_SGD').getValue('Dealt_Currency', 
            row))
    ...
    def amount_received = Mobile.getText(findTestObject('transaction_successful/Converted from'), 5, FailureHandling.CONTINUE_ON_FAILURE)
